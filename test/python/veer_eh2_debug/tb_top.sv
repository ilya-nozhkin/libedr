// This file is a modified version of the tb_top.sv file from the VeeR-EH2 project:
// https://github.com/chipsalliance/Cores-VeeR-EH2/blob/main/testbench/tb_top.sv
// It remains subject to its original license specified below.
// All changes are marked by <EDR>...</EDR> comments.
...
// <EDR>
// Driving the clock internally to get rid of the top-level .cpp file.
// `ifdef VERILATOR
...
// </EDR>
    // <EDR>
    edr_Error edr_error_api;
    edr_Context edr_ctx_api;
    edr_ExecutionGate edr_execution_gate_api;
    edr_ByteStreamTunnel edr_tunnel_api;

    edr_Jtag edr_jtag_api;
    wire system_is_idle;
    wire test_tunnel_resetn;
    // </EDR>
...
`endif
    ...
    .i_cpu_run_req          ('0),
    // <EDR>
    .o_debug_mode_status    (system_is_idle),
    // </EDR>
    .o_cpu_run_ack          (),
...
// <EDR>
// Replacing the bitbang JTAG with the libedr version.
assign srstn = 1;
assign trstn = rst_l;

test_tunnel test_tunnel_instance (
    .clk_o(core_clk),
    .resetn_o(test_tunnel_resetn),

    .error_o(edr_error_api),
    .context_o(edr_ctx_api),
    .execution_gate_o(edr_execution_gate_api),
    .tunnel_o(edr_tunnel_api)
);

edr_jtag edr_jtag_instance (
    .clk_i(core_clk),
    .system_is_idle_i(system_is_idle),
    ...
    .context_i(edr_ctx_api),
    .execution_gate_i(edr_execution_gate_api),
    .jtag_o(edr_jtag_api)
);

initial begin
  wait (edr_jtag_api != null);
  edr_tunnel_api.RegisterDriver(edr_jtag_api);

  edr_tunnel_api.StartServer(edr_error_api);
  if (edr_error_api.Fail()) begin
    $display("Failed to start serving the byte stream tunnel: ", edr_error_api.Message());
    $finish(1);
  end

  while (edr_tunnel_api.IsAlive()) begin
    @(posedge core_clk);
  end
