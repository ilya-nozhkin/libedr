...
``libedr`` defines abstract asynchronous C++, C, Python and SystemVerilog APIs for some hardware interfaces and provides implementations for some common targets such as RTL simulations.
...
If tests and Python, SystemVerilog and C APIs are not needed, then this should be enough:
...
* Driving APB from SystemVerilog: [examples/systemverilog/apb](examples/systemverilog/apb)
* Driving APB from Python: [examples/python/apb](examples/python/apb)
* Driving JTAG TAP from Python: [examples/python/jtag_idcode](examples/python/jtag_idcode)
